...
const FrDialogs = {
	...
	assignToSelectedPerson: function (faces,  clusterLenght, selectedName, callback) {
		return $.when(this._getMessageTemplate()).then(function ($tmpl) {
			var dialogName = 'fr-assign-to-selected-dialog';
			var dialogId = '#' + dialogName;
			var $dlg = $tmpl.octemplate({
				dialog_name: dialogName,
				title: t('facerecognition', 'Add group to {name}', {name: selectedName}),
				message: t('facerecognition', 'Unassigned groups left to rename: {count}', {count: clusterLenght}),
				type: 'none'
			});
			...
			var buttonlist = [{
				...
				text: t('facerecognition', 'Add to {name}', {name: selectedName}),
				click: function () {
					$(dialogId).ocdialog('close');
					if (callback !== undefined) {
						callback(true, selectedName.trim());
					}
				}
			}];
			...
			$(dialogId).ocdialog({
				...
				open: function () {
					// Automatically focus the first button ("Skip for now")
					$(dialogId).parent().find('.oc-dialog-buttonrow button').first().focus();
				}
			});
		});
	},
